...
def generate_homo(img1, img2, homo_parameter, img_truth=None, transform=None):
    # if transform is not None:
    #     img1, img2 = transform(image=img1)['image'], transform(image=img2)['image']
    img1, img2 = img1 / 255, img2 / 255 # normalize
    if img_truth is not None and (img_truth.size > 0):
        img_truth = img_truth/255
    ...
    if img_truth is not None and (img_truth.size > 0):
        img_truth = img_truth[top_left[1]-marginal:bottom_right[1]+marginal+1, top_left[0]-marginal:bottom_right[0]+marginal+1, :]
    ...
    if img_truth is not None and (img_truth.size > 0):
        warped_img_truth = cv2.warpPerspective(img_truth, H_inverse, (img1.shape[1], img1.shape[0]))
        patch_img_truth = warped_img_truth[top_left[1]:bottom_right[1]+1, top_left[0]:bottom_right[0]+1, :]
        patch_img_truth = torch.from_numpy(patch_img_truth).float().permute(2, 0, 1)
    ...
    if img_truth is not None and (img_truth.size > 0):
        return patch_img1, patch_img2, ground_truth, org_pts, dst_pts, large_img1, large_img2, patch_img_truth
    
    return patch_img1, patch_img2, ground_truth, org_pts, dst_pts, large_img1, large_img2

def use_homo(img1, img2, homo_parameter, org_pts, dst_pts, img_truth=None):
