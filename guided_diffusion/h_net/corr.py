...
from .corr_implement_gyh import *
from .utils import local_correlation_gyh
...
class LocalCorr:
    ...
    def __call__(self, coords, window_size=9):
        map2_warp = self.warp(coords, self.map2, self.H, self.W)
        # print(' torch.max(map2_warp), torch.min(map2_warp)', torch.max(map2_warp), torch.min(map2_warp))
        # print('map2_warp.shape', map2_warp.shape)
        # exit()
        # corr = corr_implement.FunctionCorrelation(self.map1, map2_warp)
        corr = FunctionCorrelation(self.map1, map2_warp, window_size)
        return corr
...
class LocalCorr_gyh:
    ...
    def __call__(self, coords):
        ...
        # print('map2_warp.shape', map2_warp.shape)
        # exit()
        corr = local_correlation_gyh(self.map1, map2_warp, 4, map2_warp.shape[-1])
        # corr = corr_implement.FunctionCorrelation(self.map1, map2_warp)
        return corr
